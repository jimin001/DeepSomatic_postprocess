...
# 0. filter for 'PASS' variants
bcftools filter -i 'FILTER="PASS"' ${germline_VCF} | bgzip > ${OUTPUT_DIR}/${sample}/${sample}_deepvariant_pass_only.vcf.gz
bcftools filter -i 'FILTER="PASS"' ${somatic_VCF} | bgzip > ${OUTPUT_DIR}/${sample}/${sample}_deepsomatic_pass_only.vcf.gz

bcftools index -t ${OUTPUT_DIR}/${sample}/${sample}_deepvariant_pass_only.vcf.gz
bcftools index -t ${OUTPUT_DIR}/${sample}/${sample}_deepsomatic_pass_only.vcf.gz

# 1. subtract germline variants from somatic variant set
bcftools isec -p ${OUTPUT_DIR}/${sample} ${OUTPUT_DIR}/${sample}/${sample}_deepsomatic_pass_only.vcf.gz ${OUTPUT_DIR}/${sample}/${sample}_deepvariant_pass_only.vcf.gz
...
bgzip ${sample}_deepvariant_only.vcf
bgzip ${sample}_intersection.vcf

bcftools index -t ${sample}_deepsomatic_only.vcf.gz

# 2. filter out variants GQ<20, DP<10
bcftools view -e 'FMT/GQ<20' ${OUTPUT_DIR}/${sample}/${sample}_deepsomatic_only.vcf.gz | bcftools view -e 'FMT/DP<10' | bgzip > ${OUTPUT_DIR}/${sample}/${sample}_deepsomatic_only_GQ20_DP10.vcf.gz
bcftools index -t ${OUTPUT_DIR}/${sample}/${sample}_deepsomatic_only_GQ20_DP10.vcf.gz
...
bcftools view ${sample}_deepsomatic_pass_only.vcf.gz | grep -v "#" | wc -l
